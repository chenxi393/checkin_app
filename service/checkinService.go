...
func CreateCheckin(viewCreatCheckin *viewmodel.CreateCheckin) (*model.Checkin, error) {
	...
	if err != nil {
		zap.L().Sugar().Errorf("用户'%v'创建签到失败", checkin.CreatorID)
		tx.RollBack() //回滚
		return nil, err
	}
	...
	if len(shouldCheckInClassLst) == 0 || err != nil {
		zap.L().Sugar().Errorf("签到'%v'的课程'%v'的签到班级无法获取", checkin.CheckinID, checkin.LessonID)
		tx.RollBack()
		return nil, err
	}
	...
	for i := range shouldCheckInClassLst {
		...
		if err != nil {
			zap.L().Sugar().Errorf( "签到'%v'根据班级'%v'获取学生列表失败", checkin.CheckinID, class.ClassId)
			tx.RollBack()
			return nil, err
		}
		for j := range shouldCheckInStuLst {
			...
			if err != nil {
				zap.L().Sugar().Errorf( "签到'%v'添加签到记录'%v'失败", checkin.CheckinID, checkinRec)
				tx.RollBack()
				return nil, err
			}
		}
	}
	zap.L().Sugar().Infof("用户'%v'创建签到'%v'成功", checkin.CreatorID, checkin.CheckinID)
	err = tx.Commit()
	if err != nil {
		return nil, err
	}
...
func StuCheckin(viewStuCheckin *viewmodel.Checkin) (res int, err error) {
	...
	if err != nil {
		zap.L().Sugar().Errorf( "用户'%v'获取签到'%v'失败", viewStuCheckin.UserID, viewStuCheckin.CheckinID)
	...
	if err != nil {
		...
		if err.Error() == "record not found" {
			zap.L().Sugar().Errorf( "签到'%v'的应签到列表中无此学生'%v'", viewStuCheckin.CheckinID, viewStuCheckin.UserID)
			return 4, app.ErrCheckinRecNotExist
		}
		zap.L().Sugar().Errorf( "获取签到记录'%v'失败", viewStuCheckin.CheckinID+viewStuCheckin.UserID)
	...
	if checkin.CheckinCode != viewStuCheckin.CheckinCode {
		zap.L().Sugar().Errorf( "用户'%v'签到'%v'时的签到码'%v'错误", viewStuCheckin.UserID, viewStuCheckin.CheckinID, viewStuCheckin.CheckinCode)
		return 6, app.ErrCheckinCode
	}
	//5.检查签到时间是否过期
	if checkin.EndTime.Before(time.Now()) {
		zap.L().Sugar().Errorf( "用户'%v'签到'%v'时签到过期，过期时间为'%v'", viewStuCheckin.UserID, viewStuCheckin.CheckinID, checkin.EndTime)
		return 5, app.ErrCheckinExpired
	}
	//6.检查是否重复签到
	if checkinRec.State == 1 {
		zap.L().Sugar().Errorf("用户'%v'签到'%v'时重复签到", viewStuCheckin.UserID, viewStuCheckin.CheckinID)
	...
	if distance > 100 {
		zap.L().Sugar().Errorf( "用户'%v'签到'%v'时超出签到范围，用户签到范围为'%v','%v'，签到范围为'%v','%v'",
	...
	if err != nil {
		zap.L().Sugar().Errorf( "更新用户'%v'的签到状态失败,签到为'%v'", viewStuCheckin.UserID, viewStuCheckin.CheckinID)
		return 2, app.ErrCheckinUpdateState
	}
	zap.L().Sugar().Infof("用户'%v'签到成功，签到为'%v'", viewStuCheckin.UserID, viewStuCheckin.CheckinID)
	return 1, nil
}
...
func GetCheckinDetails(checkinID string) (checkinDetails *viewmodel.CheckinDetailsResponse, err error) {
	...
	if err != nil {
		zap.L().Sugar().Errorf( "签到'%v'的签到记录获取失败", checkinID)
	...
	for i := range checkinRecLst {
		...
		if err != nil {
			zap.L().Sugar().Errorf( "获取用户'%v'的班级失败", checkinRec.UserID)
	...
	zap.L().Sugar().Infof("获取签到'%v'成功", checkinID)
	return
}
...
func GetCreatedCheckInList(creatorID string) (listResponse []viewmodel.ListResponse, err error) {
	...
	if err != nil {
		zap.L().Sugar().Errorf( "获取用户'%v'创建的签到失败", creatorID)
	...
	for i := range checkinList {
		...
		if err != nil {
			zap.L().Sugar().Errorf( "获取用户'%v'创建的课程'%v'失败", creatorID, lesson.LessonID)
	...
	zap.L().Sugar().Infof("获取用户'%v'已创建的签到成功", creatorID)
	return
}
...
func GetCheckinRecList(userID string) (shouldCheckInList []viewmodel.ListResponse, err error) {
	...
	if err != nil {
		zap.L().Sugar().Errorf("获取用户'%v'的签到记录失败", userID)
		return nil, app.ErrCheckinRecGet
	}
	...
	for i := range checkinRecLst {
		...
		if err != nil {
			zap.L().Sugar().Errorf( "获取签到'%v'失败", checkinRec.CheckinID)
			return nil, app.ErrCheckinGet
		}
		endTime := checkin.EndTime
		checkinState := 2
		...
		if err != nil {
			zap.L().Sugar().Errorf( "获取课程'%v'失败", checkin.LessonID)
			return nil, app.ErrCheckinLessonGet
		}
	...
	zap.L().Sugar().Infof("获取用户'%v'的签到记录成功", userID)
	return
}
