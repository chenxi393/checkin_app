...
func InsertLesson(lesson *model.Lesson, classLesson []model.ClassLesson) error {
	...
	if err != nil {
		zap.L().Sugar().Errorf("插入课程记录失败", err)
		tx.Rollback()
	...
	for _, v := range classLesson {
		...
		if err != nil {
			zap.L().Sugar().Errorf("插入中间表记录失败", err)
			tx.Rollback()
			return app.InternalServerError
		}
	}
	tx.Commit()
	zap.L().Sugar().Infof("课程、中间表记录插入成功%d")
	return err
}
...
func GetLessonList(userId string) ([]*viewmodel.ListObj, error) {
	...
	db := DB.Table("lesson").Select([]string{`lesson_id`, `lesson_name`, `created_at`}).Where("lesson_creator = ? ", userId).Find(&lesson)
	if db.RowsAffected == 0 {
		zap.L().Sugar().Errorf("指定查找的课程记录不存在", db.Error)
	...
	zap.L().Sugar().Infof("查询用户创建列表以成功%v", res)
	return res, nil
}
...
func GetJoinLessonList(classId string) ([]*viewmodel.ListObj, error) {
	...
	// 根据中间表关联查询到当前班级加入的课堂
	db := DB.Table("class_lesson").Select([]string{`lesson.lesson_name`, `lesson.created_at`, `lesson.lesson_id`}).
		...
		Find(&lesson)
	if db.RowsAffected == 0 {
		zap.L().Sugar().Errorf("查询用户所在班级加入的课堂失败", db.Error)
		return nil, app.ErrRecordNotExist
	}
	// 根据查询出的课堂id,去反查询，得到加入该课堂的相应班级
	for _, v := range lesson {
		db = DB.Table("class_lesson").Select([]string{`class_name`}).
			Joins("inner join class on class.class_id = class_lesson.class_id").
			Where("class_lesson.lesson_id = ?", v.LessonID).Find(&classLesson)
		if db.RowsAffected == 0 {
			zap.L().Sugar().Errorf("查询当前课程所拥有的班级信息失败", db.Error)
			return nil, app.ErrRecordNotExist
		}
		var tmp []string
	...
	zap.L().Sugar().Infof("获取用户加入课程成功", resListObj)
...
func UpdateLessonName(lesson *viewmodel.LessonEditor) (err error) {
	...
	if tx.Error != nil {
		zap.L().Sugar().Errorf("更新失课程名失败", tx.Error)
		tx.Rollback()
		return app.ErrUpdated
	}
	tx.Commit()
	zap.L().Sugar().Infof("更新课程名称成功%s", lesson.LessonName)
	return nil
}
...
func InsertClassLesson(classLessonSlice []model.ClassLesson) (err error) {
	...
	for _, v := range classLessonSlice {
		if tx.Create(&v).RowsAffected == 0 {
			zap.L().Sugar().Errorf("插入中间表信息失败", tx.Error)
			tx.Rollback()
	...
	tx.Commit()
	zap.L().Sugar().Infof("插入中间表信息成功%v", classLessonSlice)
	return nil
}
...
func RemoveLesson(lesson *viewmodel.LessonRemove) (err error) {
	...
	db := DB.Table("lesson").
		...
		Delete(&model.Lesson{})
	if db.RowsAffected == 0 {
		zap.L().Sugar().Errorf("删除课程记录失败", db.Error)
		tx.Rollback()
		return app.ErrDeleted
	}
	//	移除中间表中的数据
	db = DB.Table("class_lesson").
		Where("lesson_id = ?", lesson.LessonID).
		Delete(&model.ClassLesson{})
	if db.RowsAffected == 0 {
		zap.L().Sugar().Errorf("删除中间表记录失败", db.Error)
		tx.Rollback()
		return app.ErrDeleted
	}
	tx.Commit()
	zap.L().Sugar().Infof("移除课程记录成功%d", db.RowsAffected)
	return nil
}
...
func LessonCreatorIsExist(lesson *viewmodel.LessonRemove) error {
	...
	if db.Error != nil {
		zap.L().Sugar().Errorf("查询课程记录失败", db.Error)
		return app.ErrRecordNotExist
	}
	zap.L().Sugar().Infof("查询指定用户创建的课程记录成功%d", db.RowsAffected)
	return nil
}
...
func LessonIsExist(lessonParam *viewmodel.Lesson) (err error, ok bool) {
	var lesson model.Lesson
	db := DB.Select(`lesson_name`).Where("lesson_creator = ?", lessonParam.LessonCreator).
		Find(&lesson)
	if db.RowsAffected == 0 {
		zap.L().Sugar().Errorf("查询课程记录失败", db.Error)
		return app.ErrRecordNotExist, false
	}
	zap.L().Sugar().Infof("查询当前用户是否创建重复课程操作成功%d", db.RowsAffected)
...
func GetLessonInfoByLessonId(lessonId string) (err error, lesson *model.Lesson) {
	// 初始化参数，不然会报空指针异常
	lesson = &model.Lesson{}
	db := DB.Table("lesson").Find(&lesson, "lesson_id = ?", lessonId)
	if db.RowsAffected == 0 {
		zap.L().Sugar().Errorf("查询课程记录失败", db.Error)
		return app.ErrRecordNotExist, nil
	}
	zap.L().Sugar().Infof("查询课程信息成功%d", db.RowsAffected)
	return nil, lesson
}
...
func DeleteClassIdByLessonId(lessonId string) (err error) {
	...
	tx.Table("class_lesson").Where("lesson_id = ?", lessonId).
		Delete(&model.ClassLesson{})
	if tx.Error != nil {
		zap.L().Sugar().Errorf("删除班级id失败", tx.Error)
		tx.Rollback()
		return app.ErrDeleted
	}
	tx.Commit()
	zap.L().Sugar().Infof("删除班级id是啊比%d", tx.RowsAffected)
	return nil
}
