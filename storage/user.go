...
func IsExistUser(field, param string, a *model.User) bool {
	isPhone := DB.Where(field+" = ?", param).First(&a)
...
func CreateUserMapper(user *model.User, classId string) error {
	...
	if db.Error != nil {
		tx.Rollback()
		zap.L().Sugar().Errorf("创建班级失败", db.Error)
		return app.InternalServerError
	}
	zap.L().Sugar().Infof("创建班级：成功创建:%v条记录", db.RowsAffected)
	// 维护用户班级中间表
	...
	if intermediateTable.Error != nil {
		//失败则回滚事务
		tx.Rollback()
		zap.L().Sugar().Errorf("维护中间表失败", intermediateTable.Error)
		return app.InternalServerError
	}
	zap.L().Sugar().Infof("维护班级用户中间表：成功创建:%v条记录", intermediateTable.RowsAffected)
	tx.Commit()
	return nil
}
...
func UpdateUserMapper(updateUser viewmodel.UpdateUserInfoRequest) error {
	result := DB.Model(model.User{}).Where("user_id = ?", updateUser.UserId).Updates(model.User{
	...
	if result.Error != nil {
		zap.L().Sugar().Errorf("修改用户信息失败", result.Error)
		return app.InternalServerError
	}
	zap.L().Sugar().Infof("修改用户信息：成功修改 %v 条记录", result.RowsAffected)
	return nil
}

// UpdateEmailMapper 修改邮箱
func UpdateEmailMapper(updateEmail viewmodel.UpdateEmailRequest) error {
	result := DB.Model(&model.User{}).Where("user_id = ?", updateEmail.UserId).Update("email", updateEmail.Email)
	if result.Error != nil {
		zap.L().Sugar().Errorf("修改邮箱失败", result.Error)
		return app.InternalServerError
	}
	zap.L().Sugar().Infof("修改邮箱：成功修改 %v 条记录", result.RowsAffected)
	return nil
}

// UpdateNickNameMapper 修改昵称
func UpdateNickNameMapper(updateNickName viewmodel.UpdateNickNameRequest) error {
	result1 := DB.Model(&model.User{}).Where("user_id = ?", updateNickName.UserId).Update("nick_name", updateNickName.NickName)
	if result1.Error != nil {
		zap.L().Sugar().Errorf("修改昵称失败", result1.Error)
		return app.InternalServerError
	}
	zap.L().Sugar().Infof("修改昵称：成功修改 %v 条记录", result1.RowsAffected)
	return nil
}
...
func GetPasswordById(userID string) (string, error) {
	user := &model.User{}
	result := DB.Select("password").Where("user_id = ?", userID).Find(&user)
	if result.Error != nil {
		zap.L().Sugar().Errorf("查找用户密码失败", result.Error)
		return "", app.InternalServerError
	}
...
func UpdatePasswordByFieldMapper(filed, condition, password string) error {
	//更新密码
	result := DB.Model(&model.User{}).Where(filed+" = ?", condition).Update("password", password)
	if result.Error != nil {
		zap.L().Sugar().Errorf("修改密码失败", result.Error)
		return app.InternalServerError
	}
	return nil
}
...
func GetEmailByPhone(phone string) (string, error) {
	user := &model.User{}
	//Select只查询email字段
	result := DB.Select("email").Where("phone = ?", phone).Find(&user)
	if result.Error != nil {
		zap.L().Sugar().Errorf("查找用户邮箱失败", result.Error)
		return "", app.InternalServerError
	}
...
func GetUserInfoByPhone(phone string) (*viewmodel.UserInfo, error) {
	...
	if scan.RowsAffected == 0 {
		zap.L().Sugar().Errorf("找不到账号为: %v 的信息", phone, scan.Error)
		return &viewmodel.UserInfo{}, app.ErrAccountDoesNotExist
	}

	zap.L().Sugar().Infof("查找到账号为: %v 的信息", phone)
	return result, nil
}
