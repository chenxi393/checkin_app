...
func Logging() gin.HandlerFunc {
	return func(context *gin.Context) {
		...
		if err := json.Unmarshal(blw.body.Bytes(), &response); err != nil {
			zap.L().Sugar().Errorf("response body 不能解码到 model。响应结构体正文: `%s` err: %e", blw.body.Bytes(), err)
		...
		zap.L().Sugar().Infof("%-13s | %-12s | %s %s | {code: %d, message: %s}", latency, ip, pad.Right(method, 5, ""), path, code, message)
	}
}
